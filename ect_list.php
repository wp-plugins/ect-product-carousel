...
<td>			
	...
	<?php 
	$frm=unserialize($s->option_value);
	$cat='';			
...
$SecArr=$wpdb->get_results("select sectionID,sectionName from sections where sectionName like '".ucwords($frm['sec_id'])."'");	
					...
					echo (($frm['slide_images_from']=='store') ? 'Whole store' : (($frm['slide_images_from']=='recom') ? 'Recommended Products' : ucwords($frm['sec_id'])));
					?>
					</td>	
					<?php 
					$p=explode('_',$s->option_name);	
					end($p);?>	
